...
myFocusedBorderColor = "#ff77aa"

myWorkspaces= ["main","vim","serv","bg","up","media","rndm"]
...
polybarPP = dynamicLogWithPP $ def {
  ppCurrent = wrap ("%{F#ff77aa}[%{F-}%{F#ffbbee}") "%{F-}%{F#ff77aa}]%{F-}"
  , ppHidden  = wrap ("%{F" ++ "#ffbbee" ++ "} ") " %{F-}"
  , ppHiddenNoWindows  = wrap ("%{F" ++ "#6B5A68" ++ "} ") " %{F-}"
  , ppUrgent = wrap ("%{F#8c414f}[%{F-}%{F#ffbbee}") "%{F-}%{F#8c414f}]%{F-}"
  , ppSep              = "  "
  , ppLayout = wrap ("%{F#6B5A68}[%{F-}%{F#ff77aa}") "%{F-}%{F#6B5A68}]%{F-}"
  , ppTitle            = (\str -> "")
...
myManageHook = manageDocks <+> (composeAll . concat $
  ...
  , [className =? c --> doShift   "main"   | c <- myWeb        ]
  , [className =? c --> doShift   "rndm"   | c <- myMisc       ]
  , [className =? c --> viewShift "media"  | c <- myIm         ]
  , [role      =? r --> doShift   "serv"   | r <- myServ       ]
  , [resource  =? r --> viewShift "vim"    | r <- myEd         ]
...
myKeys = \conf -> mkKeymap conf $
    ...
    , ("M-<F4>", namedScratchpadAction scratchpads  "keepassx")
    , ("M-<F5>", namedScratchpadAction scratchpads  "gotop")
    , ("M-<F6>", namedScratchpadAction scratchpads  "telegram")
